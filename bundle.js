...
function run(edgeType = "all", query = "") {
  ...
  const sim = simulation_default(nodes).force("charge", manyBody_default().strength((d) => d.kind === "module" ? -300 : -140)).force("link", link_default(links).id(nodeKey).distance((d) => 42 + 2 * Math.min(d.source.degree, d.target.degree)).strength(0.15)).force("collide", collide_default().radius((d) => 6 + Math.sqrt(2 + d.degree)).iterations(2)).force("x", x_default2().strength(0.06)).force("y", y_default2().strength(0.06));
  currentSim = sim;
