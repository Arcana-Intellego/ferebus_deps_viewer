...
var zoom = zoom_default2().on("zoom", (ev) => {
  ...
  if (onZoomRepaint) onZoomRepaint();
});
svg.call(zoom);
function run(edgeType = "all", query = "") {
  ...
  function arrowPath(d) {
    const { x2: x22, y2: y22, ux, uy } = endpoints(d);
    const ARW = 6 / zoomK;
    ...
    return `M${x22},${y22} L${b1x},${b1y} L${b2x},${b2y} Z`;
  }
  function repaint() {
    link.attr("x1", (d) => endpoints(d).x1).attr("y1", (d) => endpoints(d).y1).attr("x2", (d) => endpoints(d).x2).attr("y2", (d) => endpoints(d).y2);
